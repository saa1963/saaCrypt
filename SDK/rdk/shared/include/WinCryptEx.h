...
/*!
 * \file $RCSfile$
 * \version $Revision: 3342 $
 * \date $Date: 2012-07-03 09:52:05 +0400 (Tue, 03 Jul 2012) $
...
#define BCRYPT_CP_GOST_R3411_ALGORITHM CAT_L(CP_GOST_R3411_ALGORITHM)
#define BCRYPT_CP_GOST_28147_ALGORITHM CAT_L(CP_GOST_28147_ALGORITHM)
#define BCRYPT_CP_GOST_R3410EL_ALGORITHM CAT_L(CP_GOST_R3410EL_ALGORITHM)
...
typedef struct _SELFTEST_HEADER {
    ...
    DWORD UnwrittenChecksums; 
			  /*!< TODO: Лёша, а на русском? Слабо?
			   * TODO: dedal - translate, please.
			   * if memory provided is not enough  
 ...
 * наборов можно с помощью вызова SetProvParam (PP_FAST_CODE_FUNCS), 
 * в который передаётся данная структура, либо, на уровне CPC, 
 * при инициализации криптографического модуля, путём передачи 
 * данной структуры в инициализатор провайдера, или при 
 * использовании криптопровайдера в пользовательском режиме, 
 * путём установки маски оптимального набора функций в реестр с 
 * помощью утилиты csptest, с параметрами '-speed -type ALL -mode REG'. 
 * Такой вызов запишет маску в реестр или его эмуляцию в параметр 
 * '/Parameters/MMXFuncs'.
 * 
 ...
 * с использованием даных CRYPT_DIVERSBLOB, размещённых в пердзаголовке пакета. 
 ...
 * Диветсифицированное значение ключей сохраняется для использования для обработки последующих пакетов.
 ...
 * Использование предварительной диверсификации ключа, определяемая флагом CP_CHP_STARTMIX, не предусмотрено.
 * В случае использования флагов CP_CHP_MULTIPACKET|CP_CHP_STARTMIX возвращается ошибка NTE_BAD_FLAGS.
 ...
 *  CSP_MultiPacket, в параметре cbData передаётся число структур в массиве (не должно превышать 16), параметр dwBufLen 
 *  (в случае CPEncrypt()) не используется и должен быть равен нулю. 
 *  В каждой из структур CSP_MultiPacket до вызова CPEncrypt()/CPDecrypt() в 
 ...
 *  \brief Структура для передачи параметров в случае мультипакетной обработки
 * (использования флага CP_CHP_MULTIPACKET).
 * Данная структура передаёт пакет как линейный буфер либо как IOVEC если в 
 * параметре dwFlags устаны флаги CP_CRYPT_HASH_PACKET, CP_CRYPT_DATA_IOVEC и 
 * CP_CHP_MULTIPACKET.
 *  
 */
typedef struct CSP_Multipacket_ {
    BYTE*	    pbData;	/*!< указатель на линейный пакет, указатель на IOVEC. */
    DWORD	    cbData;	/*!< длина линейного пакета, число элементов IOVEC. */
    DWORD	    dwBufLen;	/*!< длина буфера линейного пакета. */
    DWORD	    dwResult;	/*!< результат обработки пакета.  */
} CSP_Multipacket;

/*! \ingroup CryptMultipacket
 * \brief Макрос возвращает указатель на пакет с номером n в массиве пакетов.
 *
 * Параметры:
...
/*! \ingroup CryptMultipacket
 * \brief Макрос возвращает длину пакета с номером n в массиве пакетов 
 * либо число координат в представлении пакета вектором ввода вывода.
 *
 * Параметры:
 * - p - указатель на первый элемент в массиве структур CSP_Multipacket;
 * - n - номер структуры в массиве пакетов.
...
/*! \ingroup CryptMultipacket
 * \brief Макрос возвращает длину буфера линейного пакета с номером n в массиве пакетов. 
 *
 * Параметры:
 * - p - указатель на первый элемент в массиве структур CSP_Multipacket;
 * - n - номер структуры в массиве пакетов.
...
#define MultiPacket_LEN(p,n) (((CSP_Multipacket*)p)[n].cbData)
/*! \ingroup CryptMultipacket
 * \brief Макрос возвращает результат оработки пакета с номером n в массиве пакетов. 
 * Для функций CPEncrypt(), CPCEncrypt(), CPDecrypt(), CPCDecrypt() 
 * перед их вызовом а это поле устанавливается удиница. 
