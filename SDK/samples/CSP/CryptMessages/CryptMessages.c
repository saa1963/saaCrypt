...
// - контейнер с ключом AT_KEYEXCHANGE в провайдере PROV_GOST_2001_DH
...
static void DecryptMessage(BYTE *pbEncryptedBlob, DWORD cbEncryptedBlob, HCERTSTORE hStoreHandle);
...
int main(void)
{
    ...
    BYTE pbContent[] = "Security is our business"; // Сообщение
    DWORD cbContent = sizeof(pbContent);	   // Длина сообщения, включая конечный 0
    ...
    DecryptMessage(pbEncryptedBlob, cbEncryptedBlob,	hStoreHandle);
...
// этого сообщения; и hStoreHandle, дескриптор
// открытого хранилища сертификатов.
void DecryptMessage( 
